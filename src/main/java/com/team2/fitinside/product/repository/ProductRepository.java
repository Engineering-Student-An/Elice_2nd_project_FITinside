...
//    Page<Product> searchByKeywordAndCategoryAndIsDeletedFalse(@Param("category") Category category, @Param("keyword") String keyword, Pageable pageable);
    @Query("SELECT p FROM Product p JOIN p.categories c WHERE p.isDeleted = false AND c = :category AND (p.productName LIKE :keyword OR p.info LIKE :keyword)")
    Page<Product> searchByKeywordAndCategoryAndIsDeletedFalse(Category category, String keyword, Pageable pageable);
